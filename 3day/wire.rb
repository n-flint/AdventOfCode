...
class Wire
  # should find out how to input this file into two arrays of strings
  # input_file = File.read('wire_path.rb').split(',')

  def self.make_directions(wire_path_1, wire_path_2)
    wire_1 = find_coordinates(wire_path_1)
    wire_2 = find_coordinates(wire_path_2)
    intersections = wire_1 & wire_2

    closest_coordinate(intersections)
  end

  # Takes array of directions, outputs an array of matching coordinates for those directions
  def self.find_coordinates(directions)
    coordinates = { x: 0, y: 0}
    line_points = []
  ...
  def self.closest_coordinate(input)
    require 'pry'; binding.pry
    test = []
    input.each do |coords|
      unless coords == {:x=>0, :y=>0}
        # require 'pry'; binding.pry
        test << coords[:d]= (coords[:x].abs + coords[:y].abs)
      end
    end
    test.sort.first
  end
...
# puts Wire.make_directions(path_1, path_2)
p Wire.make_directions(path_3, path_4)
# p Wire.make_directions(path_5, path_6)
# p Wire.make_directions(puzzle_wire_1, puzzle_wire_2)